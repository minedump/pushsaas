...
export const API_INTRO = {
  ...
  scopesNote:
    "Ключу выдаются только нужные разделы — Рассылки/Шаблоны/Подписчики/Автоматизации, чекбоксами при создании ключа (раздел «API» админки). Запрос к эндпоинту вне выданных ключу разделов отвечает 403 { \"error\": \"api key missing scope: <раздел>\" }, ключ при этом остаётся валидным — это не 401.",
  liquidNote:
