...
// Управление подписчиком из раздела «Подписчики»: приостановка push и
...
export async function POST(req: Request) {
  const { projectId, subscriberId, action, channel, active, phone, email } = (await req.json().catch(() => ({}))) as {
    projectId?: string;
    subscriberId?: string;
    action?: "pause" | "resume" | "channel";
    channel?: "sms" | "email";
    active?: boolean;
    phone?: string | null;
    email?: string | null;
