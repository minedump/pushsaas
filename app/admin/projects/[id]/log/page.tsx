...
export default async function LogPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="max-w-4xl mx-auto">
      <h1 className="text-2xl font-semibold">{project.name} · Журнал автоматизаций</h1>
      <p className="text-ink-muted mt-0 text-[13px]">Сработки триггеров: событийные, по API и по вебхукам.</p>

      {log.length === 0 ? (
        <Card className="mt-4 text-ink-muted">Пока пусто — здесь появятся запуски автоматизаций.</Card>
      ) : (
