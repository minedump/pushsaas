...
function Card({ config, variant }: { config: PromptConfig; variant: "mobile" | "desktop" }) {
  const base = variant === "mobile" ? promptMobileStyle(config) : promptDesktopStyle(config);
  const actionsRow = variant === "mobile";
  return (
    <div
      className="shadow-lg"
      style={{
